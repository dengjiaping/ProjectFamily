 ...
 *
 */
public class EnhancedGifDecoder{
	...
	private List<EnhancedGifView> mViewList = new ArrayList<EnhancedGifView>();

	public EnhancedGifDecoder(){
	    this.action = new GifAction(){
			@Override
	...
	public void addView(EnhancedGifView view) {
	...
	public void removeView(EnhancedGifView view) {
		synchronized (mViewList) {
			if (mViewList.contains(view)) {
	...
	public EnhancedGifDecoder(GifAction action){
	    this.action = action;
	}
...
//			}
	...
	public GifFrame next() {
		GifFrame frame = do_next();
		Bitmap bitmap = null;
		if (frame.image != null) {
			bitmap = frame.image;
		} else {
			bitmap = BitmapFactory.decodeFile(frame.imageName);
		}

		synchronized (mViewList) {
			Iterator<EnhancedGifView> it = mViewList.iterator();
			while (it.hasNext()) {
				EnhancedGifView view = it.next();
	...
	//			matrix.postScale(scaleWidth, scaleHeight);
				...
				view.setCurrentImage(newBitmap);
				view.triggerRedraw();
			}
		}
