...
import jp.co.cyberagent.android.gpuimage.GPUImageCustomFilter;
...
import jp.co.cyberagent.android.gpuimage.GPUImageMissEtikateFilter;
...
/**
 * GPUImage滤镜工具，可以选择多种滤镜效果
 * @author wangshichao
 *
 */
public class GPUImageFilterTools {
	private static final String TAG = "GPUIMAGEFILTERTOOLS";
	private static final boolean DEBUG = true;

	/**
	 * 根据选择的滤镜的类型判断，选择创建相应的滤镜，标注释的为本应用目前选择使用的滤镜
	 * @param context
	 * @param type 滤镜类型
	 * @return 该滤镜
	 */
	public static GPUImageFilter createFilterForType(final Context context,
		...
		case LOOKUP_AMATORKA:
			...
			return amatorka;
		case NASHVILLE: // 乡村
			return createCustomFilter(context, "FilterNashville.fsh");
		case AMARO: // 怀旧
			return createCustomFilter(context, "FilterAmaro.fsh");
		case SIERRA:// 梦想
			return createCustomFilter(context, "FilterSierra.fsh");
		case WALDEN:// 日光
			return createCustomFilter(context, "FilterWalden.fsh");
		case XPROII:// 清明
			return createCustomFilter(context, "FilterXProII.fsh");
		case MISSETIKATE:// 蓝光 （还有问题）
			return new GPUImageMissEtikateFilter();
		case OPENING:// 水粉
						// 需要GPUImageRGBErosionFilter和GPUImageRGBDilationFilter组合滤镜
			return null;
		case TOON:// 彩色素描
					// 需要GPUImageSobelEdgeDetectionFilter
					// 和GPUImage3x3ConvolutionFilter 组合滤镜（暂不需要）
			return new GPUImageToonFilter();
		case SKETCH:// 黑白素描
			return createCustomFilter(context, "FilterSketch.fsh");
		case ORIGINAL: // 原始
	...
	/**
	 * 创建通过载入shader滤镜脚本创建自定义的滤镜
	 * @param context
	 * @param filterFileName 滤镜脚本文件名称
	 * @return
	 */
	private static GPUImageFilter createCustomFilter(Context context,
			String filterFileName) {
		try {
			if (filterFileName != null) {
				String fragmentShaderString = GPUImageFilter.loadShader(
						filterFileName, context);
				Log.v(TAG, "GPUImageFilter fragmentShaderString="
						+ fragmentShaderString);
				// GPUImageFilter filter = new
				// GPUImageFilter(GPUImageFilter.NO_FILTER_VERTEX_SHADER,
				// fragmentShaderString);
				GPUImageCustomFilter filter = new GPUImageCustomFilter(
						fragmentShaderString);
				if (filter.getFilterFileName() == filterFileName) {
					filter.mChangeFilter = false;
					return null;
				} else {
					filter.setFilterFileName(filterFileName);
					filter.mChangeFilter = true;
				}
				Log.v(TAG, "GPUImageCustomFilter filter=" + filter);
				return filter;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static GPUImageFilter createBlendFilter(Context context,
		...
		try {
			...
			filter.setBitmap(BitmapFactory.decodeResource(
					context.getResources(), R.drawable.cx_fa_hb_icon));
			return filter;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
...
//	 }
	 ...
	 protected float range(final int percentage, final float start, final
	 float end) {
