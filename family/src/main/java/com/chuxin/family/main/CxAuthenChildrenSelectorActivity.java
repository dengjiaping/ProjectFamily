...
public class CxAuthenChildrenSelectorActivity extends CxRootActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.cx_fa_activity_main_login_children_selector);
...
//		initTitle();
	...
	private void init() {
		ImageView mKidImg = (ImageView) findViewById(R.id.cx_fa_main_login_kids_selector_kid_iv);
		ImageView mNoKidImg = (ImageView) findViewById(R.id.cx_fa_main_login_kids_selector_nokid_iv);

		mKidImg.setOnClickListener(clickListener);
		mNoKidImg.setOnClickListener(clickListener);
...
//	}
	...
	OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.cx_fa_main_login_kids_selector_kid_iv:
				DialogUtil du = DialogUtil.getInstance();
				du.getSimpleDialog(CxAuthenChildrenSelectorActivity.this, "使用亲子版",
						"即将使用亲子版，是否确定？", null, null).show();
				du.setOnSureClickListener(new OnSureClickListener() {

					@Override
					public void surePress() {
						try {
							DialogUtil.getInstance().getLoadingDialogShow(CxAuthenChildrenSelectorActivity.this, -1);
							UserApi.getInstance().updateUserProfile(null, null,null, null, null,null,null,2+"", genderCaller);
						} catch (Exception e) {
							e.printStackTrace();
				...
				//
				break;
			case R.id.cx_fa_main_login_kids_selector_nokid_iv:

				DialogUtil dul = DialogUtil.getInstance();
				dul.getSimpleDialog(CxAuthenChildrenSelectorActivity.this, "使用二人版",
						"即将使用二人专属版，是否确定？", null, null).show();
				dul.setOnSureClickListener(new OnSureClickListener() {
					...
					public void surePress() {
						try {
							DialogUtil.getInstance().getLoadingDialogShow(CxAuthenChildrenSelectorActivity.this, -1);
							UserApi.getInstance().updateUserProfile(null, null,null, null, null,null,null,1+"", genderCaller);
						} catch (Exception e) {
							e.printStackTrace();
							DialogUtil.getInstance().setLoadingDialogDismiss(null, -1, 1000);
						}
	...
	JSONCaller genderCaller = new JSONCaller() {
		...
		public int call(Object result) {
			...
			int version_type = userInitInfo.getData().getVersion_type();
			
			SharedPreferences sp = getSharedPreferences(PROFILE_FILE_NAME,Context.MODE_PRIVATE);
			Editor edit = sp.edit();
			edit.putInt("version_type", version_type);
			edit.commit();
			
			CxGlobalParams.getInstance().setVersion_type(version_type);

			Intent it = new Intent(CxAuthenChildrenSelectorActivity.this,CxMain.class);
	...
	private void displayResultInfo(String info, int number) {
		...
		msg.obj = info;
		msg.arg1 = number;
		new Handler(CxAuthenChildrenSelectorActivity.this.getMainLooper()) {
			public void handleMessage(Message msg) {
				...
				ToastUtil.getSimpleToast(CxAuthenChildrenSelectorActivity.this,
