 ...
 *
 */
public class CxFamilyInfoActivity extends CxRootActivity {
...
//	private RkImageView iconImg;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cx_fa_fragment_mate_family_info);
		oppoInfo=new FamilyInfoUserInfo();
		meInfo=new FamilyInfoUserInfo();

		Intent intent = getIntent();
		int intExtra = intent.getIntExtra(CxMateParams.FAMILY_FLAG, 1);
		flag=intExtra;
		
		init();
			
		
		CxFamilyInfoCacheData cacheData=new CxFamilyInfoCacheData(this);
		...
		UserApi.getInstance().requestFamilyInfo(this, familyInfoCaller);
	...
	private void init() {
		...
		Button backBtn = (Button) findViewById(R.id.cx_fa_mate_family_info_title_menu);
		backBtn.setBackgroundResource(R.drawable.cx_fa_back_btn);
		backBtn.setText(TextUtil.getNewSpanStr(getString(R.string.cx_fa_navi_back), 16, Color.rgb(240, 236, 234)));
		backBtn.setOnClickListener(tabListener);
		
		
		familyImg = (RoundAngleImageView) findViewById(R.id.cx_fa_mate_family_info_icon);
		
//		LinearLayout familyImgLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_icon_layout);
		
		
		int screen_w = getResources().getDisplayMetrics().widthPixels; // 屏幕宽度					
	    int imgWidth 	= screen_w - ScreenUtil.dip2px(this, 20)*2;				// 头像的宽 = 屏幕的宽度-左右margin（假设margin-left、margin-right都是20px）
		...
		oppoLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_oppo_layout);
		meLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_me_layout);
		LinearLayout homeLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_home_layout);
		LinearLayout kidLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_kid_layout);
		if(version_type==2){
		...
		oppoText = (TextView) findViewById(R.id.cx_fa_mate_family_info_oppo_tv);
		meText = (TextView) findViewById(R.id.cx_fa_mate_family_info_me_tv);
		
		iconOppoLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_icon_oppo_layout);
		iconMeLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_icon_me_layout);
		
		nameLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_name_layout);
		LinearLayout birthdayLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_birthday_layout);
		consLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_cons_layout);
		LinearLayout phoneLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_phone_layout);
		
		oppoOtherLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_other_oppo_layout);
		meOtherLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_other_me_layout);
		
		remarkLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_remark_layout);
		LinearLayout moreLayout = (LinearLayout) findViewById(R.id.cx_fa_mate_family_info_user_more_layout);
		
		nameView = (View) findViewById(R.id.cx_fa_mate_family_info_user_name_view);
		consView = (View) findViewById(R.id.cx_fa_mate_family_info_user_cons_view);
		remarkView = (View) findViewById(R.id.cx_fa_mate_family_info_user_remark_view);
		
		iconOppoImg = (CxImageView) findViewById(R.id.cx_fa_mate_family_info_user_icon_oppo_riv);		
		iconMeImg = (CxImageView) findViewById(R.id.cx_fa_mate_family_info_user_icon_me_riv);		
		
		nameText = (TextView) findViewById(R.id.cx_fa_mate_family_info_user_name_tv);		
		birthdayText = (TextView) findViewById(R.id.cx_fa_mate_family_info_user_birthday_tv);
		consText = (TextView) findViewById(R.id.cx_fa_mate_family_info_user_cons_tv);
		consImg = (ImageView) findViewById(R.id.cx_fa_mate_family_info_user_cons_iv);
		phoneText = (TextView) findViewById(R.id.cx_fa_mate_family_info_user_phone_tv);
		remarkText = (TextView) findViewById(R.id.cx_fa_mate_family_info_user_remark_tv);
...
//			iconImg.setImageResource(RkResourceDarwable.getInstance().dr_chat_icon_small_me);
	...
	OnClickListener tabListener=new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.cx_fa_mate_family_info_home_layout:
				...
				Intent intent=new Intent(CxFamilyInfoActivity.this,CxNbOurHome.class);
			...
			case R.id.cx_fa_mate_family_info_kid_layout:
				CxGlobalParams.getInstance().setChangeMenu(CxMain.KID);
				finish();
	...
	OnClickListener userListener=new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.cx_fa_mate_family_info_icon: //家庭背景
				Intent intent = new Intent(CxFamilyInfoActivity.this, ActivitySelectPhoto.class);
			...
			case R.id.cx_fa_mate_family_info_user_icon_me_layout:
				Intent selectImageForHead = new Intent(CxFamilyInfoActivity.this, ActivitySelectPhoto.class);
				ActivitySelectPhoto.kIsCallPhotoZoom =true;
				ActivitySelectPhoto.kIsCallFilter = false;
				ActivitySelectPhoto.kIsCallSysCamera = true;
				ActivitySelectPhoto.kChoseSingle = true;
	...
	protected void showBirthdayDialog() {
		...
		View view = View.inflate(CxFamilyInfoActivity.this, R.layout.cx_fa_widget_accounting_date_dialog,null);
		...
		final Dialog dialog=new Dialog(CxFamilyInfoActivity.this, R.style.simple_dialog);		
	...
	protected void showMoreDialog() {
		...
		if (mAddView == null) {
			mAddView = CxFamilyInfoActivity.this.getLayoutInflater().inflate(R.layout.cx_fa_activity_mateprofile_add, null);  
	...
	private void showDialog(final TextView view,final String title,int type,final int from){
		...
		nameDu.getEditDialog(CxFamilyInfoActivity.this, view.getText().toString(), title, null, null,type).show();
	...
	private void showAddTitleDialog(){
		DialogUtil nameDu = DialogUtil.getInstance();
		nameDu.getEditDialog(CxFamilyInfoActivity.this,null,"比如“爱好”" , "设置自定义资料项", null, null,0).show();
		nameDu.setOnEidtSureClickListener(new OnEidtSureClickListener() {					
			@Override
			public void editSurePress(String str) {
				if(flag==1){
	...
	private void addView(final String title,final String value,int from,int flag){
			...
			final View tempView = CxFamilyInfoActivity.this.getLayoutInflater().inflate(R.layout.cx_fa_fragment_mate_family_info_item, null);
		    ...
		    delImg.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					du.getSimpleDialog(CxFamilyInfoActivity.this, title, "确认删除？", null, null).show();
...
//			String text=RkFamilyInfoFragment.this.getActivity().getString(R.string.cx_fa_mate_family_info_family_icon_fail);
			...
			showResponseToast(getString(R.string.cx_fa_mate_family_info_family_icon_success), 1);
...
//			}
			...
			str=getString(R.string.cx_fa_net_response_code_fail);
...
//			}
	...
	private void resetData(final int resetId){
		new Handler(CxFamilyInfoActivity.this.getMainLooper()) {
	...
	private void showResponseToast(String info,int number) {
		...
		new Handler(CxFamilyInfoActivity.this.getMainLooper()) {
			public void handleMessage(Message msg) {
				...
				ToastUtil.getSimpleToast(CxFamilyInfoActivity.this, id,
...
//		        }
	...
	private String getSign(String strMmdd){
	    ...
	    String arr[] = { CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Sagittarius),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Capricorn),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Aquarius),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Pisces),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Aries),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Taurus),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Gemini),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Cance),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Leo),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Virgo),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Libra),
	    						 CxFamilyInfoActivity.this.getString(R.string.cx_fa_nls_mateprofile_Scorpio)};
