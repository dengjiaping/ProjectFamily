 ...
 * @author shichao.wang
 */
public class CxNeighbourAddInvitationImagePager extends CxRootActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        pager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int arg0) {
                ...
                mTitle.setText((mPosition + 1) + "/"
                        + CxNeighbourParam.getInstance().getAddPhotosPath().size());
            }
        ...
        if ((null != CxNeighbourParam.getInstance().getAddPhotosPath())
                && (mPosition < CxNeighbourParam.getInstance().getAddPhotosPath().size())) {
            mTitle.setText((mPosition + 1) + "/"
                    + CxNeighbourParam.getInstance().getAddPhotosPath().size());
            pager.setCurrentItem(mPosition);
        }
    ...
    private class ImagePagerAdapter extends PagerAdapter {
        ...
        ImagePagerAdapter() {
            ...
            mImagePathArrString = CxNeighbourParam.getInstance().getAddPhotosPath();
        }
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.cx_fa_activity_title_back:
                    CxNeighbourAddInvitationImagePager.this.finish();
                    break;
                case R.id.cx_fa_activity_title_more: // 保存照片 or 删除图片
                    mSaveBtn.setClickable(false);
                    CxNeighbourParam.getInstance().removeSpecialImage(mPosition);
                    // mImageViewAdapter.
                    // 修正mPosition
                    if (mPosition >= CxNeighbourParam.getInstance().getAddPhotosPath().size()) {
                        mPosition = CxNeighbourParam.getInstance().getAddPhotosPath().size() - 1;
                    }
                    if ((null == CxNeighbourParam.getInstance().getAddPhotosPath()) // 无图片应该关闭此界面
                            || (CxNeighbourParam.getInstance().getAddPhotosPath().size() < 1)) {
                        CxNeighbourAddInvitationImagePager.this.finish();
                        return;
                    }
