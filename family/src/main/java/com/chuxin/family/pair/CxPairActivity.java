...
public class CxPairActivity extends CxRootActivity {
	...
	private static CxPairActivity mSelf;
	...
	public static CxPairActivity getInstance(){
		return mSelf;
	}
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.cx_fa_fragment_ready_invit);
		
		CxGlobalParams mGlobalParam = CxGlobalParams.getInstance(); // 获取model的subject实例
		pairObserver = new CurrentObserver();
		List<String> tags = new ArrayList<String>();
		tags.add(CxGlobalParams.PAIR); // 结对状态
		tags.add(CxGlobalParams.SINGLE_MODE); //单人模式状态
		pairObserver.setMainThread(true);
		mGlobalParam.registerObserver(pairObserver);
		
		mMenuBtn = (Button)findViewById(R.id.cx_fa_invite_menu);
		mSendBtn1 = (Button)findViewById(R.id.cx_fa_invite_send);
		mSendBtn2 = (Button)findViewById(R.id.cx_fa_invite_send2);
...
//		mSendBtn2.setText(getString(RkResourceString.getInstance().str_pair_invite_partner_text));
		mMenuBtn.setBackgroundResource(R.drawable.cx_fa_back_btn);
		mMenuBtn.setText(TextUtil.getNewSpanStr(getString(R.string.cx_fa_navi_back), 16, Color.rgb(240, 236, 234)));
		
		mMenuBtn.setOnClickListener(clicker);
		mSendBtn1.setOnClickListener(clicker);
		mSendBtn2.setOnClickListener(clicker);
		
//		myInvitedNumberText = (TextView).findViewById(R.id.cx_fa_my_invite_number);
//		showDimesionCode = (Button).findViewById(R.id.cx_fa_dimessionbar_btn);
		
		TextView mTopText = (TextView)findViewById(R.id.cx_fa_pair_top_word_tv);
		mTopText.setText(CxResourceString.getInstance().str_pair_top_word);
		
		mInviteNumTip = (TextView)findViewById(R.id.cx_fa_invite_num_tip);
		
		sendPairRequest = (Button)findViewById(R.id.cx_fa_request_pair);
		requestNumber = (EditText)findViewById(R.id.cx_fa_request_pair_number_code);
		...
		public void onClick(View v) {
			...
			case R.id.cx_fa_request_pair: //发送结对邀请
				...
				if (TextUtils.isEmpty(requestNumber.getText().toString())) {
					Toast noneInviteNumber = Toast.makeText(CxPairActivity.this, 
							CxPairActivity.this.getString(R.string.cx_fa_input_invite_number), Toast.LENGTH_LONG);
				...
				if (requestNumber.getText().toString().equalsIgnoreCase(mInviteNumStr)
					...
					Toast noneInviteNumber = Toast.makeText(CxPairActivity.this, 
							CxPairActivity.this.getString(R.string.cx_fa_same_invitenum_with_me), Toast.LENGTH_LONG);
					noneInviteNumber.setGravity(Gravity.CENTER, 0, 0);
					noneInviteNumber.show();
					return;
				}
				...
				v.setEnabled(false);
				try {
//					RkLoadingUtil.getInstance().showLoading(RkPairActivity.this, true);
					DialogUtil.getInstance().getLoadingDialogShow(CxPairActivity.this, -1);
...
//					RkLoadingUtil.getInstance().dismissLoading();
			...
			case R.id.invite_mate_by_sms:
				...
				if (TextUtils.isEmpty(mInviteNumStr)) {
					Toast.makeText(CxPairActivity.this, getString(
			...
			case R.id.invite_mate_by_wx:
				...
				if(CxAuthenNew.api.isWXAppInstalled() && CxAuthenNew.api.isWXAppSupportAPI()){
		    		...
		    		Toast.makeText(CxPairActivity.this, 
			...
			case R.id.invite_mate_by_qr:
				...
				mInviteDialog = null;
				AlertDialog dlg = new AlertDialog.Builder(CxPairActivity.this).create();
				dlg.show();
				ImageView img = new ImageView(CxPairActivity.this);
	...
	private void invitePartner(){
		AlertDialog.Builder adb = new AlertDialog.Builder(
				new ContextThemeWrapper(CxPairActivity.this, R.style.inviteDialogCustom)
			);  
		adb.setTitle("");
		adb.setItems(new String[]{							
			...
			public void onClick(DialogInterface arg0, int arg1) {
				...
				case 1: //微信邀请
					...
					break;
				case 2: //二维码邀请
					DialogUtil.getInstance().getCodeDialogShow(CxPairActivity.this, R.drawable.cx_fa_invite_qr_code, 
				...
				/*case 1: //QQ邀请
				    ...
				    mTencent.shareToQQ(RkPairActivity.this, params, new IUiListener() {
			            
			            @Override
			            public void onError(UiError arg0) {
			                Toast.makeText(RkPairActivity.this, "发送失败", Toast.LENGTH_LONG).show();
			            }
			            
			            @Override
			            public void onComplete(JSONObject arg0) {
			                Toast.makeText(RkPairActivity.this, "发送成功", Toast.LENGTH_LONG).show();
			            }
			            
			            @Override
			            public void onCancel() {
			                Toast.makeText(RkPairActivity.this, "发送取消", Toast.LENGTH_LONG).show();
			            }
			        });
				    type=1;
					break;*/
	...
	JSONCaller myInvitedNumber = new JSONCaller() {
		...
		public int call(Object result) {
			...
			SharedPreferences sp = CxPairActivity.this.getSharedPreferences(
					CxGlobalConst.S_PAIR_CH_NAME, Context.MODE_PRIVATE);
			Editor edt = sp.edit();
	...
	JSONCaller sendInviteRequest = new JSONCaller() {
		...
		public int call(Object result) {
			DialogUtil.getInstance().setLoadingDialogDismiss(null, -1, 2000);
			new Handler(CxPairActivity.this.getMainLooper()){
				public void handleMessage(android.os.Message msg) {
...
//					RkLoadingUtil.getInstance().dismissLoading();
			...
			if (null == result) {//异常
				showMessageInToast(CxPairActivity.this.getString(R.string.cx_fa_net_err));
				return -1;
			}
			CxCall callDetail = null;
			try {
...
//			/*Intent inviteMeAction = new Intent(RkPairActivity.this, RkInvitations.class);
//			startActivity(inviteMeAction);*/
//			Intent inviteMeAction = new Intent(RkPairActivity.this, RkApproveInvitation.class);
//			inviteMeAction.putExtra("from", 2);
//			startActivity(inviteMeAction);
			return 0;
		}
	};
	...
	private void showApproveDialog(List<CxPairInitData> matchs ){
		View view = View.inflate(this, R.layout.cx_fa_widget_pair_approve_dialog,null);
		...
		iconImg.displayImage(imageLoader, data.getIcon(), 
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {			
					dialog.dismiss();
					approveDialog = DialogUtil.getInstance().getLoadingDialog(CxPairActivity.this);
					approveDialog.show();
	...
	private void showSameGenderDialog() {
		...
		du.getSimpleDialog(CxPairActivity.this, null, str, null, null).show();
		du.setOnSureClickListener(new OnSureClickListener() {
			
			@Override
			public void surePress() {
				Intent intent = new Intent(CxPairActivity.this, CxAuthenGenderSelectorActivity.class);
				startActivity(intent);
				CxPairActivity.this.overridePendingTransition(R.anim.tran_pre_in, R.anim.tran_pre_out);
				CxPairActivity.this.finish();
			}
		});
	};
	...
	private void showMessageInToast(String message){
		Handler updateMyInviteNumber = new Handler(Looper.getMainLooper()){
			public void handleMessage(android.os.Message msg) {
				...
				Toast netErrorToast = Toast.makeText(CxPairActivity.this, 
...
//					notifyPairSuccess(); //弹框确认结对成功，确定后修改RkGlobalParams中的kpair值
					...
					if(pair==0){ //未结对状态下的结对
					    ...
					    //TODO 清空聊天记录
//					    Message message = new Message(null, CxApplication.getInstance().getContext());
//					    if(null != message){
//					        message.dropAll();
//					    }
...
//					back();
	...
	private CurrentObserver pairObserver;

//	private void notifyPairSuccess(){
//		Handler updateMyInviteNumber = new Handler(Looper.getMainLooper()){
//			public void handleMessage(android.os.Message msg) {
//				AlertDialog ad = new AlertDialog.Builder(RkPairActivity.this).create();
...
//						//改变global中结对标志位
//						RkGlobalParams.getInstance().setPair(1); //进一步反馈给观察者
//						
//					}
//				});
//				
...
//	}
	...
	class CurrentObserver extends CxObserverInterface {
		...
		public void receiveUpdate(String actionTag) {
			...
			if (actionTag.equalsIgnoreCase(CxGlobalParams.SINGLE_MODE)) { // 对方发帖子成功时long polling告诉更新
	...
	protected void onDestroy() {		
		...
		if(pairObserver!=null){
			CxGlobalParams.getInstance().unRegisterObsercer(pairObserver);
			pairObserver=null;
		}
	}
