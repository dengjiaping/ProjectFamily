...
public class LocationEntry extends ChatLogEntry {
    private static final String TAG = "LocationEntry";

    private static final int VIEW_RES_ID = R.layout.cx_fa_view_chat_chatting_location_row;

    private static final int ICON_ID = R.id.cx_fa_view_chat_chatting_location_row__icon;

    private static final int CONTENT_ID = R.id.cx_fa_view_chat_chatting_location_text;

    private static final int TIMESTAMP_ID = R.id.cx_fa_view_chat_chatting_location_row__timestamp;

    private static final int DATESTAMP_ID = R.id.cx_fa_view_chat_chatting_location_row__datestamp;

    private static final int SEND_SUCCESS_BUTTON_ID = R.id.cx_fa_view_chat_chatting_location_row__exclamation;

    private static final int CHAT_VIEW_LINEARLAYOUT_ID = R.id.cx_fa_view_chat_chatting_location_row__content;

    private static final boolean OWNER_FLAG = true;

    private String mMsgText;

    public LocationEntry(Message message, Context context, boolean isShowDate) {
        super(message, context, isShowDate);
    }

    @Override
    public int getType() {
        return ENTRY_TYPE_LOCATION;
    }
    ...
    public View build(View view, ViewGroup parent) {
        ...
        LinearLayout chatLinearLayout = (LinearLayout)view
                .findViewById(getChatViewLinearLayoutId());
        ...
        LocationMessage msgLocation = (LocationMessage)mMessage;

        int msgType = msgLocation.getType();
        final int msgId = msgLocation.getMsgId();
        int msgCreatTimeStamp = msgLocation.getCreateTimestamp();
        mMsgText = msgLocation.getText();

        final float lat = msgLocation.getLat();
        final float lon = msgLocation.getLon();

        chatLinearLayout.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View v) {
                ChatFragment.getInstance().openLoactionMap(lat, lon, mMsgText);
            }
        });
        ...
        if (isOwner()) {
            int msgSendState = msgLocation.getSendSuccess();
            final ImageButton reSendBtn = (ImageButton)view.findViewById(getSendSuccessButtonId());
            ProgressBar pb = (ProgressBar)view
                    .findViewById(R.id.cx_fa_view_chat_chatting_location_row_circleProgressBar);
            if (msgSendState == 0) {
                        ...
                        .setPositiveButton(R.string.cx_fa_chat_button_resend_text,
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        reSendBtn.setVisibility(View.GONE);
                                        ChatFragment.getInstance().reSendMessage(mMsgText, 5, msgId, lat, lon);
                                    }
