...
public class CalendarGridView extends ViewGroup {
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		boolean isHeadRow = ((CalendarRowView) child).getIsHeaderRow();
		if (isHeadRow) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int cellSizeW = widthMeasureSize / 7;
		int cellSizeH = heightMeasureSize / getNumRows();

		// Remove any extra pixels since /7 is unlikely to give whole nums.
		widthMeasureSize = cellSizeW * 7;
		int totalHeight = 0;
		final int rowWidthSpec = makeMeasureSpec(widthMeasureSize, EXACTLY);
		final int rowHeightSpec = makeMeasureSpec(cellSizeH, EXACTLY);
		for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
			final View child = getChildAt(c);
			if (child.getVisibility() == View.VISIBLE) {
				if (c == 0) { // It's the header: height should be wrap_content.
					measureChild(child, rowWidthSpec,makeMeasureSpec(cellSizeW, AT_MOST));
				} else {
					CxLog.i("onMeasure", "rowWidthSpec=" + rowWidthSpec);
